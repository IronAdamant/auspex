...
| --- | --- | --- |
| 0:00–0:30 | Open [Pages](https://ironadamant.com/auspex/). First screen: **`ok` ≠ `claimOk` ≠ `claimOkProfile`** (true / false skipped / true on the redacted receipt) and the blurred auth-gated still. Labels: **Measured**, **Redacted demo**, **Stripped wall**. | Header legend + blur |
| 0:30–1:00 | The player further down is the Microsoft sign-in wall only (emails and passwords stripped). Not logged-in proof. Login doors (handoff only): [phone](../examples/auspex-ts/demo/door-phone.gif) and [desktop](../examples/auspex-ts/demo/door-desktop.gif), also at the top of the [root README](../README.md#login-doors-phone-and-desktop). | Wall label, door GIFs |
