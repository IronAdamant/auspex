...
| 4:00–5:00 | Stranger path (read only): `login --url https://…` (profile from the host) → human Save → `await-login --save-editor` → finalize only when the seed is the app session, not `idp-only-save` / `app-visible`. Frozen door: [AGENTS.md](../AGENTS.md#frozen-agent-door-sequence). Fail-closed also: `expectMatchedPublicLanding`, `hostChanged`, `stream-expired`. The phone countdown is the ~5 minute Solari token. Save before zero. Do not restart await after the paste. A slow sign-in needs a fresh login for the final Save window, not a longer token. Phone Chrome dogfood: pause/reconnect on background; remint only when the JWT is gone. The phone text field is the Auspex workaround for [Solari cookbook #80](https://github.com/solari-sdk/solari-cookbook/issues/80) (remote view will not open a phone keyboard). Optional compose: `auspex_job` (not a fourth primitive). | Not a same-session takeover |
