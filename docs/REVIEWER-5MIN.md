...
| 4:00–5:00 | Frozen door (read only): [AGENTS.md](../AGENTS.md#frozen-agent-door-sequence) — mint → human login → Save → `await-login --save-editor` → `finalize-login` → later `check`. Fail-closed: `expectMatchedPublicLanding`, `hostChanged`, `stream-expired`. | Not a same-session takeover |
