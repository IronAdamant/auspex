...
- `editor/save` 409 "not in a savable state" gets one live `POST /editor/token` (no TTL, token not kept) and one more save. If the editor is gone or the second save fails, status is `stream-expired`. A failed save does not claim cookies. If another Auspex path already owns that Save (posted or in flight), the loser is `sibling-saved` and does not report `stream-expired`.
