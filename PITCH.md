...
- Frozen agent door (not Handraise live-view takeover): mint `login --url` → human login in the door → human Save → `await-login --save-editor` → `finalize-login` only when the seed is the app session (unique expect) → later `check` / optional `--verify-with-profile`. An IdP app, especially MSAL, can already be on screen while Save stored only Microsoft or Google cookies (`idp-only-save`, kind `app-visible`). Do not finalize that jar. That stop is honest. Login worked. `ok` ≠ `claimOk` ≠ `claimOkProfile`. Fail-closed: `expectMatchedPublicLanding`, `hostChanged` remint, `stream-expired`. Five-minute skim: [docs/REVIEWER-5MIN.md](docs/REVIEWER-5MIN.md). See [AGENTS.md](AGENTS.md#frozen-agent-door-sequence). Solari `402`/`429`/`502` are not `loggedOut`/`needsHuman`.
