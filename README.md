...
This page is a seed/handoff door: you type here, and the keys go to remote Chrome. It is not a same-session VNC takeover. Show as bullets is off by default so a password manager can paste into the text field. It is not the Mousepad sandbox demo (`auspex desktop`).
