...
login mint lead-up is traced to .auspex/trace/login.jsonl (event: login only; key, profile, handoff, editor-start, editor-token). Production does not write await-login / finalize-login / check rows. mintStage ready only when VNC/token mint succeeded. If mint fails, login JSON and npx auspex trace say why (missing key, 429, 402, 503, no url, editor-start HTTP, VNC timeout, empty handoff token). Log stops when Chromium/handoff is ready; await-login / check are normal ops after that (not this log). Default npx auspex trace is the last episode plus traceSummary. --all dumps history. No tokens, cookie values, excerpts, passwords, or session ids. If mint is silent or fails, read traceSummary / auspex_trace before reminting. Not a fourth primitive.
