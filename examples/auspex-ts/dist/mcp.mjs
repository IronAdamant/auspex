...
function cookieHostsFromState(state) {
...
function idpCookiesFromHosts(hosts) {
  return hosts.some(
    (h) => hostIs(h, "login.microsoftonline.com") || hostIs(h, "login.live.com") || hostIs(h, "accounts.google.com")
  );
}
function foldedExpiresInSecFromState(state, origin, now = Date.now()) {
  if (!state || !origin) return void 0;
  const expiresMs = foldedExpiresOnMs(state, origin);
  if (expiresMs === void 0) return void 0;
  return Math.round((expiresMs - now) / 1e3);
}
function loginTraceSeedExtras(state, origin) {
  ...
  if (folded !== void 0) extras.foldedExpiresInSec = folded;
  return extras;
}
function sanitizeLoginTraceEvent(raw) {
  const out = { ts: raw.ts, event: raw.event };
...
async function appendLoginTrace(event, file = LOGIN_TRACE_PATH) {
  try {
...
function clampTraceLimit(limit) {
...
async function loadActive(file) {
  try {
...
function newEpisodeId(now = /* @__PURE__ */ new Date()) {
  const stamp = now.toISOString().replace(/[:.]/g, "-");
  return `ep-${stamp}-${Math.random().toString(36).slice(2, 6)}`;
}
function summarizeLoginTrace(events) {
...
async function recordLoginTrace(event, opts = {}) {
...
function cleanTraceText(value) {
  if (/slr_|password|eyJ[\w-]+\./i.test(value)) return void 0;
  return value;
}
async function recordPostHandoffTrace(raw) {
  ...
  const ready = [...events].reverse().find(
    (e) => e.event === "login" && e.profile === profile && (e.mintStage === "ready" || e.vncMintOk === true)
  );
...
async function readLoginTrace(opts = {}) {
  const file = opts.file ?? LOGIN_TRACE_PATH;
  ...
  let episodeId;
  if (!opts.all) {
    episodeId = [...events].reverse().find((e) => e.event === "login" && e.episodeId)?.episodeId ?? [...events].reverse().find((e) => e.episodeId)?.episodeId;
...
var LOGIN_TRACE_PATH, LOGIN_TRACE_ACTIVE_PATH, LOGIN_TRACE_LIMIT_DEFAULT, LOGIN_TRACE_LIMIT_MAX, COOKIE_HOST_CAP, FORBIDDEN_KEYS;
...
function solariFailurePayload(err) {
  ...
  return {
    ...
    ...issue.status !== void 0 ? { status: issue.status } : {},
    ...reap ? { nextCall: { tool: "auspex_reap" } } : {}
  };
}
...
var CLOSE_KILL_RECOVERY, AuspexError;
var init_errors = __esm({
  "src/errors.ts"() {
    "use strict";
    init_profile_lock();
    CLOSE_KILL_RECOVERY = "Not retryable. Free the slot with auspex_reap (or solari_browser_close / solari_kill if that MCP is loaded), then retry.";
    AuspexError = class extends Error {
      issue;
      sessionId;
      screenshotPath;
      log;
...
function profileSlugFromHost(host) {
  ...
  if (!slug || !/^[a-z0-9]/.test(slug)) return void 0;
  return slug;
}
...
function derivedProfileNext(name) {
  return `Profile name derived from URL host: ${name}. Use --profile ${name} on await-login, finalize-login, and check. Override with --profile <yours>.`;
}
var PROFILE_SLUG_ERROR, LOGIN_PROFILE_OR_URL_ERROR, PROFILE_SLUG_MAX;
var init_profile_slug = __esm({
  "src/profile-slug.ts"() {
    "use strict";
    PROFILE_SLUG_ERROR = "could not derive a profile name from url; pass --profile <yours>";
    LOGIN_PROFILE_OR_URL_ERROR = "login requires --profile <name> or --url <https> (url without profile derives a host slug)";
    PROFILE_SLUG_MAX = 48;
  }
});
...
function replayStatus(err) {
  if (err instanceof SolariError2) return err.status;
...
__export(profiles_exports, {
  ...
  PHONE_HANDOFF_NOT_TAKEOVER: () => PHONE_HANDOFF_NOT_TAKEOVER,
  ...
  publicHandoffUrl: () => publicHandoffUrl,
...
function loginInstructions(profile, urlHint, handoff, qrPath, mobileUrl, opts) {
  ...
  const loginCall = { tool: "auspex_login" };
...
async function loginProfile(name, urlHint, http, qrPath, opts) {
  let mintStage = "key-check";
  try {
    ...
    const traced = await recordLoginTrace({
      ...
      editorStartStatus: vncMint.editorStartStatus || void 0,
      tokenLastStatus: vncMint.tokenLastStatus,
      tokenTries: vncMint.tokenTries || void 0,
    ...
    await recordLoginTrace({
      ...
      profile: name,
      mintStage,
      urlPresent: issue.code === "NoHandoffUrl" ? false : void 0,
...
function boundedExpectAt(hay, index, length) {
...
function previousWordStartsUpper(hay, index) {
  let i = index - 1;
...
function finalizeLoginGuide(profile) {
  const name = profile.trim() || "<name>";
  const saved = name === "<name>" ? void 0 : savedCheckForProfile(name);
  ...
  const nextCall = { tool: "auspex_finalize_login" };
  if (name !== "<name>") nextCall.profile = name;
...
function remintLoginGuide(profile) {
  const name = profile.trim() || "<name>";
...
function claimOkProfileReuseNext(verify, existing) {
  if (verify?.claimOkProfile === void 0) return existing;
  const gate = verify.claimOkProfile === true ? `${CLAIM_OK_PROFILE_REUSE_GATE} claimOkProfile=true \u2014 profile reuse is evidenced by this field.` : `${CLAIM_OK_PROFILE_REUSE_GATE} claimOkProfile=false \u2014 do not reuse this seed. Remint or finalize-login while the token is live. ${DEAD_FOLD_VWP_BAN}`;
  return existing ? `${existing} ${gate}` : gate;
}
function saveEditorMissedFold(opts) {
...
function overlaySaveEditorGuidance(opts) {
  return { text: overlaySaveEditorNext(opts), nextCall: opts.nextCall };
}
function overlaySaveEditorNext(opts) {
  if (!saveEditorMissedFold(opts)) return opts.next;
  const why = !opts.editorSave ? "editorFold did not refresh folded sessionStorage." : !opts.editorSave.ok ? `editorSave failed (${opts.editorSave.status}${opts.editorSave.error ? `: ${opts.editorSave.error}` : ""}).` : opts.editorFold ? `editorFold.${opts.editorFold.reason} did not refresh folded sessionStorage.` : "editorFold missing after editorSave; leftover sessionStorage is not a fresh capture.";
...
function weakSeedGuide(profile, seed) {
  if (seed?.sessionStorageStale) {
    const remint = remintLoginGuide(profile);
    return {
      text: `profile ${profile} has stale folded sessionStorage expiresOn (past or within 5m; leftover count is not a fresh capture). --save-editor does not refresh folded sessionStorage. ${DEAD_FOLD_VWP_BAN} ${remint.text} finalize-login now only if the live editor tab is still on the app dashboard with a valid session. Remint auspex_login if finalize-login returns needsHuman. ${finalizeLoginGuidance(profile)}`,
      nextCall: remint.nextCall
    };
  }
  return {
    text: `profile ${profile} has cookies/origins but no counted sessionStorage. Finalize-login NOW while the token is live. ${DEAD_FOLD_VWP_BAN} Remint auspex_login if finalize-login returns needsHuman. ${finalizeLoginGuidance(profile)}`,
    nextCall: finalizeLoginGuide(profile).nextCall
  };
}
function emptyProfileGuide(profile) {
  const name = profile.trim() || "<name>";
...
function loginWaitAwaitOpts(opts = {}) {
  return {
    ...opts.sinceVersion !== void 0 ? { sinceVersion: opts.sinceVersion } : {},
    ...opts.url !== void 0 ? { url: opts.url } : {},
    saveEditor: true
  };
}
function inspectOriginForAwait(opts) {
  const raw = opts.url?.trim() || savedCheckForProfile(opts.name)?.url;
  if (!raw) return void 0;
  if (isPublicMarketingUrl(raw)) return void 0;
  try {
...
function awaitGuide(status, profile, version, seed) {
  if (status === "completed") {
    ...
    const counted = seed.sessionStorage !== void 0;
    const originNote = counted ? "" : " Inspect did not count sessionStorage (no origin forwarded). Unknown sessionStorage is not weakSeed.";
...
async function liveAwaitLogin(name, opts = {}) {
  ...
  try {
    ...
    if (outcome) {
      await recordPostHandoffTrace({ profile: result.name, ...outcome }).catch(() => void 0);
    }
    return result;
  } finally {
...
var EMPTY_PROFILE_SEED_ERROR, EMPTY_PROFILE_SAVE_ERROR, EMPTY_ORIGIN_SAVE_ERROR, PROFILE_EDITOR_OPEN_ERROR, HANDOFF_POLL_MS, AWAIT_LOGIN_DEFAULT_MS, AWAIT_LOGIN_MIN_MS, AWAIT_LOGIN_MAX_MS, DEAD_FOLD_VWP_BAN, CLAIM_OK_PROFILE_REUSE_GATE, SAVE_NOT_FOLD_NOW;
var init_profile_persist = __esm({
  "src/profile-persist.ts"() {
    ...
    CLAIM_OK_PROFILE_REUSE_GATE = "Reuse gate is claimOkProfile, not ok. ok=true is not enough to treat the profile as reusable.";
...
// src/profile-host-advice.ts
init_profiles();
init_profile_slug();
init_saved_checks();
init_sso();
var PROFILE_HOST_MISMATCH_MARK = "Profile host mismatch:";
function profileHostMismatchLead(profile, suggested, url) {
  return `${PROFILE_HOST_MISMATCH_MARK} --profile ${profile} is not the host slug for ${url} (suggestedProfile ${suggested}). profileHostMatch is false. Do not reuse this profile for the new host. Remint with npx auspex login --profile ${suggested} --url ${url} (or omit --profile and let the host slug win). This command still ran (soft advise, not a refuse). Do not follow later steps that pass --profile ${profile} for this host.`;
}
function savedProfileOwnsHost(profile, hostname) {
  const saved = savedCheckForProfile(profile);
  if (!saved?.url) return false;
  try {
    return hostIs(hostname, new URL(saved.url).hostname);
...
function adviseProfileHost(opts) {
  const profile = opts.profile?.trim();
  const url = opts.url?.trim();
  if (!profile || !url) return void 0;
  let suggested;
  let hostname;
  try {
...
function stampProfileHostAdvice(result, opts) {
  ...
  const next = prior.startsWith(PROFILE_HOST_MISMATCH_MARK) ? prior : prior ? `${advice.nextLead} ${prior}` : advice.nextLead;
  return {
    ...result,
    profileHostMatch: false,
...
function stampLoginHost(result, url) {
  return stampProfileHostAdvice(result, { profile: result.name, url });
}
async function resolveProfileHostUrl(opts) {
...
async function stampAwaitLoginHost(result, opts) {
...
var auspexLoginInputObject = z4.object({
  profile: profileNameSchema.optional().describe("Profile name to create or reuse. Omit when url is set to derive a host slug (app.example.com \u2192 app-example-com). Explicit profile wins."),
  ...
  wait: z4.boolean().optional().describe("If true, wait for Save then run saveEditor (same as await-login --save-editor). Empty Save is not success.")
});
var auspexLoginInputSchema = auspexLoginInputObject.superRefine((val, ctx) => {
  if (!val.profile && !val.url) {
    ctx.addIssue({ code: z4.ZodIssueCode.custom, message: "auspex_login requires profile or url" });
  }
});
...
var auspexTraceInputSchema = z4.object({
  ...
  all: z4.boolean().optional().describe("Dump mixed history instead of the last login episode")
});
...
init_timeout();
init_paths();
function checkLoggedOutGuide(profile, cookies) {
...
function expectMatchedPublicLandingGuide(profile) {
  const name = profile?.trim() || "<yours>";
  const text = `Expect text was found, but the live URL is not a persistable app page (/, /landing, /login, /signup, or /auth). ${PUBLIC_PROFILE_SAVE_ERROR}. reason expectMatchedPublicLanding means ok is false and matched is false; no profile bytes were written. Choose an expect that appears only on the logged-in app surface and does not appear in public marketing copy. A capitalized word does not match inside a capitalized phrase (Dashboard does not match One Dashboard); a full marketing phrase still matches and is not saved from a public URL. Run finalize-login --profile ${name} --url <persistable-app-url> --expect <unique-logged-in-text>.`;
  const nextCall = { tool: "auspex_finalize_login" };
  if (profile?.trim()) nextCall.profile = profile.trim();
  return { text, nextCall };
}
...
async function writeFittedScreenshot(abs) {
  const png = await readFile7(abs);
  const fitted = fitPngUnderCap(png, MAX_IMAGE_BYTES);
  if (fitted !== png) await writeFile6(abs, fitted);
}
...
async function buildReceiptToolContent(payload, screenshotPath) {
  ...
  try {
    const buf = await readFile8(resolveScreenshotPath(screenshotPath));
    if (buf.length === 0) {
...
async function profileStatus(opts, deps) {
  ...
  if (result.needsHuman || result.reason === "needsHuman") {
    const loginCall = { tool: "auspex_login", profile };
    return {
      ok: false,
      reason: "needsHuman",
      profile,
