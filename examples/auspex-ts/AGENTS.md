...
- **ConsistencyHub agent recipe** (dogfood saved check): `login --profile consistencyhub` → human completes Microsoft + OneDrive consent in handoff → Save → `await-login --save-editor` (does **not** refresh folded sessionStorage unless `editorFold.ok`; may warn if count is 0 or folded `expiresOn` is stale) → if `next` says stale/weakSeed: remint or finalize-now, do not run `--verify-with-profile` on a dead fold → `finalize-login --profile consistencyhub` **while the token is valid** → then `check --name consistencyhub`. Do not use console Save or `--save-editor` alone. Optional: `--verify-with-profile` (`claimOkProfile`). **Verified 2026-09-18:** After reseed v20, `check --name consistencyhub --verify-with-profile` → `ok=true`, `claimOkProfile=true`.
