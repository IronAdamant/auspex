...
- `auspex_profiles` — list names/ids, version, and whether storage is populated. After a saved login has been used and tested, ask the human whether testing is done and the login may be purged. Purge only after the human agrees (`--purge <name> --yes`, or MCP `purge` + `humanAgree: true`). On the desktop, the same wipe runs when that profile has been idle for 30 minutes. A use resets that profile's 30-minute clock. One site at a time. Username and password stay in the local page fields only, and those fields are cleared after paste or Save. They are not included in the agent message. Other profiles stay. No username, password, or Solari key is in the payload. The human pastes URL, username, and password on `docs/phone.html` or the desktop thin client `docs/desktop.html`. The desktop Solari key stays in that browser or gitignored `.auspex/operator-key`.
