...
`--sso` clicks Microsoft, then Google, then a generic Sign in with … button. Use with `--profile`. `--sso-provider` pins a vendor. `--profile` attaches the Solari profile on the **default** context (do not open a fresh context — that drops the pool seed). Empty seeds (0 cookies and 0 origins) fail closed unless `--sso`. `--save-profile` writes the live session via Solari `save-profile` and **refuses an empty overwrite**. `record`+`profile` is forbidden unless `--allow-record-profile`.
