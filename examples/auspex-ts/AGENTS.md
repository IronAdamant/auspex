...
- `auspex_profile_status` — `loggedIn` vs `loggedOut` vs `needsHuman` vs **`weakSeed`** vs **`emptySave`**. **`emptySave`** = profile not found or empty. **`weakSeed`** is ConsistencyHub (name / profile / `consistencyhub.io`) with a counted `sessionStorage === 0`. Other cookie-only landings are `loggedOut`. Default path uses **one** browser session (inspect only when there is no URL; otherwise one live check). Live probe never uses `--sso` or `--record`. Re-seed is human SSO once. Never type a Microsoft or Google password/OTP. Path `/` is `loggedOut` unless expect matched. If ConsistencyHub is logged out, report `loggedOut` and skip.
