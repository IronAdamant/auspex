...
**Mint debug (for agents):** `login` writes a redacted JSONL lead-up log (gitignored `.auspex/trace/login.jsonl`). If Chromium never comes up, read `traceSummary` on the login JSON or run `npx auspex trace` **before reminting**. The summary names the stop (missing key, 429, 402, 503, no URL, editor-start HTTP, VNC timeout). The log stops when the handoff is ready. Never tokens or passwords. Not a fourth primitive.
