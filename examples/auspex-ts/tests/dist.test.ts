...
test("source tool-copy keeps fail-closed leads and triad phrases", () => {
  ...
  assert.match(LOGIN_DESCRIPTION, /phone\.html/)
  assert.match(LOGIN_DESCRIPTION, /Clear empties the whole field/)
  assert.equal(LOGIN_DESCRIPTION.includes("door.html"), false)
...
test("built dist/mcp.mjs keeps fail-closed gates (CI builds; not committed)", () => {
  ...
  assert.match(dist, /Clear empties the whole field/)
  assert.equal(dist.includes("door.html"), false)
