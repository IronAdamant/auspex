...
function assertTypingDoor(html: string, label: string) {
  ...
  const withoutSavePaste = html.replace(/function savePasteLine\(name\) \{[\s\S]*?\n      \}/, "")
  assert.equal(/\blocalStorage\b/.test(withoutSavePaste), false, `${label} no localStorage API`)
...
test("phone and desktop doors mount Solari and one typing field", () => {
  ...
  assert.match(chooser, /cookies and localStorage only/)
  assert.match(chooser, /sessionStorage still needs finalize-login/)
  assert.match(chooser, /logged-in app surface, not marketing/)
  assert.match(chooser, /finalize-login/)
  assert.match(chooser, /hostChanged/)
  assert.match(chooser, /expectMatchedPublicLanding/)
  assert.equal(/microsoft/i.test(chooser), false)
  assert.equal(chooser.includes("One Dashboard"), false)
  ...
  assert.equal(/microsoft/i.test(phone), false)
  assert.equal(/microsoft/i.test(desktop), false)
