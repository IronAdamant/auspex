...
test("docs/phone.html has a real text field and loads the local noVNC client", () => {
  ...
  assert.match(html, /Tap a field in Solari's Chrome, then type here/)
  assert.match(html, /You're controlling Solari's remote Chrome/)
  assert.match(html, /Saved login data stays on Solari/)
  assert.match(html, /does not refresh folded sessionStorage/)
  assert.match(html, /GET editor HTTP 401/)
  assert.match(html, /Opening Solari's remote Chrome/)
...
test("phoneSavePaste is a line any agent chat can run", () => {
  ...
  assert.match(line, /Clipboard Save is not the jar/)
  assert.match(line, /do not kill it/)
  assert.match(line, /cannot lengthen the Solari token/)
