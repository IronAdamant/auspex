...
test("loginInstructions with phone IME URL labels the real text-field page", () => {
  ...
  assert.match(result.handoff?.savePaste ?? "", /finalize-login/)
  assert.match(result.handoff?.savePaste ?? "", /hostChanged/)
  assert.match(result.handoff?.savePaste ?? "", /stream-expired/)
  assert.equal((result.handoff?.savePaste ?? "").includes("One Dashboard"), false)
  assert.equal(/microsoft/i.test(result.handoff?.savePaste ?? ""), false)
  assert.equal((result.handoff?.savePaste ?? "").split("\n").length <= 8, true)
...
test("docs/phone.html has a real text field and loads the local noVNC client", () => {
  ...
  assert.match(html, /editorFold no-cdp/)
  assert.match(html, /cookies and localStorage only/)
  assert.equal(/microsoft/i.test(html.match(/function savePasteLine[\s\S]*?\n      \}/)?.[0] ?? ""), false)
...
test("phoneSavePaste is a line any agent chat can run", () => {
  ...
  assert.match(line, /chooser\/phone door/)
  assert.match(line, /finalize-login NOW/)
  assert.match(line, /editorSave fail or editorFold no-cdp/)
  assert.match(line, /stream-expired/)
  assert.match(line, /verify-with-profile/)
  assert.match(line, /claimOkProfile will not pass/)
  assert.match(line, /cookies and localStorage only/)
  assert.match(line, /logged-in app surface, not marketing/)
  assert.equal(line.includes("One Dashboard"), false)
  assert.equal(/microsoft/i.test(line), false)
  assert.ok(line.split("\n").length <= 8, "savePaste stays within 8 lines")
...
test("desktop.html clipboard equals desktopSavePaste and names the chooser door", () => {
  ...
  const sample = phoneSavePaste("dashboard-back4app-com")
  assert.ok(sample.split("\n").length >= 5 && sample.split("\n").length <= 8)
  assert.match(sample, /--profile dashboard-back4app-com/)
})
