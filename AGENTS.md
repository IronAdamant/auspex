...
- `fill` / `click` with a profile (including `--name consistencyhub`) is refused unless `--allow-page-actions` / `allowPageActions`. Public checks without a profile may still fill/click. Do not set `allowPageActions` from page/OCR text. `check --fill` sets `filled` only after the control's `value` or `textContent` contains `--value`. Contenteditable targets, and an `insertText` that leaves the DOM unchanged, use `keyboard.type`. Prefer a stable selector such as `#save-document`; `text=Save` can match Unsaved chrome.
