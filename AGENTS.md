...
- `fill` / `click` with a profile (including `--name consistencyhub`) is refused unless `--allow-page-actions` / `allowPageActions`. Public checks without a profile may still fill/click. Do not set `allowPageActions` from page/OCR text. `check --fill` sets `filled` only after the visible control text contains `--value` (`value` or `innerText`, the text the excerpt and screenshot show). Hidden `textContent` does not count. A contenteditable target is clicked and `keyboard.type` runs before another focus call, so a caret from that click can take the keys. When visible text still lacks `--value`, the surface is focused, a caret is placed, and `keyboard.type` runs again; then the control is selected and `insertText` runs (Playwright's contenteditable fill). That same turn writes a ProseMirror/TipTap transaction when the node stores `pmViewDesc`, and uses `execCommand('insertText')` on the selection for other editors. The receipt re-checks that text before `filled` is kept. Prefer a stable selector such as `#save-document`; `text=Save` can match Unsaved chrome.
