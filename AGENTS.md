...
# Human: open handoff.url (phone.html; same page on a phone or a computer). handoff.mobileUrl is that page. tap Save on that page. Clear empties the whole field. Do not intern-ping. Do not open Solari (GET editor HTTP 401).
...
Three primitives: browser check, sandbox verify, named sandbox desktop demo. Login, finalize-login, profile-status, reap, and trace are the auth + hygiene doors. No fourth primitive. `auspex_desktop` (Mousepad) is not the user's Mac. The login door is `docs/phone.html`.
...
1. **Mint** `login --url <https>` (derives `--profile` host slug; override `--profile <yours>`). The door is `handoff.url` (`phone.html`). Same page on a phone or a computer.
2. **Human logs in** in the door / VNC window. Secrets never appear on the agent line, chat, MCP, or receipts.
3. **Human Save** on the phone door. Paste the copied line in chat; the IME is cleared.
...
The login door is `phone.html`. It works on a phone or a computer. There is no separate desktop door.
