...
- `auspex_await_login` / `auspex await-login` — wait until Save stored cookies or origins. Empty Save is not success. Soft-warns if profile has cookies/origins but no sessionStorage (common with auth-gated SaaS after console Save).
