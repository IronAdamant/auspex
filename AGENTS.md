...
- `auspex_login` / `auspex login` — create/reuse a named profile and return a **single-use login-handoff URL**. Returns **mobile-first handoff packet** with `handoff.url` (handoff URL), `handoff.openOnPhone` (human instruction), `handoff.oneLiner` (SMS/email paste line), `handoff.qrPath` (generated QR PNG under run dir). `--url` is a start hint in the handoff reason (Solari POST is still `{ reason }`). Show handoff packet to the human; they sign in from desktop or mobile (agent never handles the password). Do not ping the user. Then `auspex_await_login` (a version bump with 0 cookies is **not** success), then `auspex_finalize_login` (unknown profiles need `--url` and `--expect`), then `auspex_check`. Do not skip finalize-login after Save.
