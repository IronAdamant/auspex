...
| `reason` | string | `matched` \| `loggedOut` \| `needsHuman` \| `mismatch` \| `network` \| `recordedLoggedIn` \| `expectMatchedPublicLanding` |
...
| `next` | string | Structured agent guidance for `loggedOut`, `needsHuman`, `expectMatchedPublicLanding`, `--verify-with-profile` reuse-gate (`claimOkProfile`), Save-is-not-fold, or a profile/host mismatch (`profileHostMatch` false) |
...
| `verify` | object | Sandbox result (`ok`, `claimOk`, `errors`, `claimErrors`, optional `claimOkProfile` / `claimErrorsProfile` / `claimProfileSessionId`, `runDir`; `skipped` on `loggedOut` / `needsHuman` / `expectMatchedPublicLanding`) |
